...
function OnboardingView({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-app)' }}>
      ...
      {/* ── 브랜딩 ── */}
      <div style={{ padding: '28px 24px 10px', textAlign: 'center', flexShrink: 0 }}>
        <div style={{ fontSize: 32, fontWeight: 900, color: 'var(--text-accent)', letterSpacing: '-0.04em', lineHeight: 1 }}>
          사주로
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 5, fontWeight: 500 }}>
      ...
      >
        {SLIDES.map((s, i) => (
          <div key={i} style={{
            ...
            {/* 타이틀 */}
            <div style={{ fontSize: 21, fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1.3, marginBottom: 10, whiteSpace: 'pre-line' }}>
              {s.title}
            </div>

            {/* 설명 */}
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.65, marginBottom: 14, flex: 1 }}>
              {s.desc}
            </div>
      ...
      {/* ── 인디케이터 ── */}
      <div style={{ display: 'flex', justifyContent: 'center', gap: 6, padding: '8px 0 2px', flexShrink: 0 }}>
        {SLIDES.map((_, i) => (
          <div key={i} style={{
      ...
      {/* ── 신뢰 지표 ── */}
      <div style={{ display: 'flex', padding: '8px 20px 4px', flexShrink: 0 }}>
        {[
          { num: '9대', label: '고전문헌' },
          ...
          <div key={s.label} style={{
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-accent)' }}>{s.num}</div>
