/**
 * 음력 ↔ 양력 변환 유틸리티
 * 표준 만세력 데이터 기반 (1900–2100)
 *
 * 인코딩:
 *   bit 0-11 : 1~12월 대소월 (1=30일, 0=29일), bit0=1월
 *   bit 12-15: 윤달 월 번호 (0=윤달 없음)
 *   bit 16   : 윤달 대소 (1=30일, 0=29일)
 *
 * 기준점: 양력 1900-01-31 = 음력 1900년 1월 1일
 */
...
const SOLAR_BASE = new Date(1900, 0, 31) // 양력 1900-01-31 = 음력 1900/1/1

/** 해당 음력 연도의 윤달 월 번호 (0이면 윤달 없음) */
function leapMonth(year: number): number {
  return LUNAR_INFO[year - 1900] & 0xF
}

/** 해당 음력 연도의 특정 월 일수 (29 or 30) */
function monthDays(year: number, month: number): number {
  return (LUNAR_INFO[year - 1900] >> (month + 3)) & 1 ? 30 : 29
}

/** 해당 음력 연도의 윤달 일수 (29 or 30) */
function leapDays(year: number): number {
  if (!leapMonth(year)) return 0
  return (LUNAR_INFO[year - 1900] >> 16) & 1 ? 30 : 29
}
...
export function lunarToSolar(
  ...
  for (let m = 1; m < lunarMonth; m++) {
    ...
    if (m === lm) offset += leapDays(lunarYear)
  }
  ...
  return new Date(SOLAR_BASE.getTime() + offset * 86400000)
}
